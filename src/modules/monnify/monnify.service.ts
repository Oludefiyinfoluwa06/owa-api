...
export class MonnifyService {
  ...
  private async withRetries<T>(
    ...
    for (let i = 0; i < attempts; i++) {
      try {
        ...
        this.logger.warn(
          `Monnify call failed (attempt ${i + 1}/${attempts}), retrying in ${wait}ms`,
  ...
  private async getAccessToken(): Promise<string> {
    ...
    const accessToken =
      data?.responseBody?.accessToken || data?.response?.accessToken;
    const expiresIn =
      data?.responseBody?.expiresIn || data?.response?.expiresIn || 300;
  ...
  async initializeTransaction(
    ...
    this.logger.debug('Initializing Monnify transaction', {
      url,
      paymentReference,
      amount,
    });
  ...
  async initBankPayment(transactionReference: string, bankCode: string) {
    ...
    this.logger.debug('Initializing bank transfer', {
      url,
  ...
  async authorizeCardOtp(
    ...
    this.logger.debug('Authorizing card OTP', {
      url,
      tokenId,
      transactionReference,
    });
    const resp = await this.withRetries(async () => {
      try {
        ...
        this.logger.error(
          'Monnify authorizeCardOtp error',
          e?.response?.status,
          e?.response?.data || e?.message,
        );
        throw e;
  ...
  async createReservedAccount(userId: string) {
    ...
    this.logger.debug('Creating Monnify reserved account', {
      url,
      accountReference,
      token,
    });
    ...
    const resp = await this.withRetries(async () => {
      try {
        ...
        this.logger.error(
          'Monnify create reserved account error',
          e?.response?.status,
          e?.response?.data || e?.message,
        );
